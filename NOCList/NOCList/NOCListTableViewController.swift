...
class NOCListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let nOCAgent = agents[indexPath.row]
        cell.detailTextLabel?.text = nOCAgent.coverName
        cell.textLabel?.text = nOCAgent.realName
    ...
    private func compromisedCount() -> String {
        ...
        for aNOCAgent in agents {
            if aNOCAgent.compromised {
              compromisedCount += 1
            }
